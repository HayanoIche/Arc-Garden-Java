DROP TABLE tb_missoes_concluidas;
DROP TABLE tb_itens_comprados;
DROP TABLE tb_loja;
DROP TABLE tb_itens_loja;
...
CREATE TABLE tb_itens_loja (
   item_loja_id INTEGER GENERATED ALWAYS AS IDENTITY,
   nome VARCHAR2(120) NOT NULL,
   tipo VARCHAR2(20) NOT NULL,
   preco_agua NUMERIC(5,2) NOT NULL,

   CONSTRAINT tb_itens_loja_pk PRIMARY KEY (item_loja_id),

   CONSTRAINT tb_itens_loja_ck
       CHECK (
...
CONSTRAINT tb_loja_item_loja_fk
 FOREIGN KEY (item_loja_id)
     REFERENCES tb_itens_loja (item_loja_id)
);

CREATE TABLE tb_itens_comprados (
        item_comprado_id INTEGER GENERATED ALWAYS AS IDENTITY,
        data_compra DATE NOT NULL,
        quantidade INTEGER NOT NULL,
        usuario_id INTEGER NOT NULL,
        item_loja_id INTEGER NOT NULL,

        CONSTRAINT tb_itens_comprados_pk PRIMARY KEY (item_comprado_id),

        CONSTRAINT tb_itens_comprados_usuario_fk
            FOREIGN KEY (usuario_id)
                REFERENCES tb_usuarios (usuario_id),

        CONSTRAINT tb_itens_comprados_item_loja_fk
            FOREIGN KEY (item_loja_id)
                REFERENCES tb_itens_loja (item_loja_id),

        CONSTRAINT tb_itens_comprados_ck
            CHECK (quantidade > 0)
);

CREATE TABLE tb_missoes_concluidas (
           missao_concluida_id INTEGER GENERATED ALWAYS AS IDENTITY,
           missao_id INTEGER NOT NULL,
           usuario_id INTEGER NOT NULL,
           data_conclusao DATE NOT NULL,
           pontos_ganhos INTEGER NOT NULL,

           CONSTRAINT tb_missoes_concluidas_pk PRIMARY KEY (missao_concluida_id),

           CONSTRAINT tb_missoes_concluidas_missoes_fk
               FOREIGN KEY (missao_id)
                   REFERENCES tb_missoes (missao_id),

           CONSTRAINT tb_missoes_concluidas_usuario_fk
               FOREIGN KEY (usuario_id)
                   REFERENCES tb_usuarios (usuario_id),

           CONSTRAINT tb_missoes_concluidas_ck
               CHECK (
                   pontos_ganhos >= 0
